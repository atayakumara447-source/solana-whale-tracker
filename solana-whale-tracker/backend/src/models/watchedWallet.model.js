...
/**
 * Mencari SATU wallet yang dipantau berdasarkan alamatnya.
 * Dipakai oleh halaman Wallet Detail untuk mengambil info dasar
 * (label, sejak kapan dipantau) sebelum menampilkan riwayat
 * transaksi dan analytics-nya.
 */
export const findWatchedWalletByAddress = async (walletAddress) => {
  const query = `SELECT * FROM watched_wallets WHERE wallet_address = $1;`;
  const result = await pool.query(query, [walletAddress]);
  return result.rows[0]; // undefined jika wallet tidak dipantau
};
